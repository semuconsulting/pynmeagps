...
from pynmeagps import NMEAReader, NMEATypeError, NMEAStreamError
...
class StreamTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.streamNMEASTARTUP = open(os.path.join(dirname, 'pygpsdata-nmeastartup.log'), 'rb')
        self.streamNMEAFOO1 = open(os.path.join(dirname, 'pygpsdata-nmeafoo1.log'), 'rb')
        self.streamNMEAFOO2 = open(os.path.join(dirname, 'pygpsdata-nmeafoo2.log'), 'rb')
    ...
    def testNMEA4(self):  # stream of NMEA v4.10 device (u-blox M9N) (NB everything after PUBX is synthetic)
        EXPECTED_RESULTS = (
            ...
            '<NMEA(GLGSV, numMsg=3, msgNum=2, numSV=10, svid_01=75, elv_01=37.0, az_01=57, cno_01=, svid_02=76, elv_02=78.0, az_02=303, cno_02=18, svid_03=77, elv_03=27.0, az_03=253, cno_03=21, svid_04=84, elv_04=19.0, az_04=18, cno_04=, signalID=1)>',
            ...
            '<NMEA(GNGLL, lat=53.450657, NS=N, lon=-2.24041, EW=W, time=10:36:07, status=A, posMode=A)>',
            ...
            '<NMEA(GPXTE, warn=A, lock=A, ctrkerr=4.07, dirs=L, disUnit=N)>',
            '<NMEA(GPVBW, wlspd=12.3, wtspd=0.07, wstatus=A, glspd=11.78, gtspd=0.12, gstatus=A)>',
            '<NMEA(GPSTN, talkerId=34)>',
            '<NMEA(GPBWC, fixutc=220516, lat=51.500333, NS=N, lon=-0.772333, EW=W, bearT=213.8, bearTu=T, bearM=218.0, bearMu=M, dist=4.6, distUnit=N, wpt=EGLM)>',
            ...
            '<NMEA(GPAPB, blnkwarn=A, cycwarn=A, ctrkerr=0.1, dirs=R, ctrkUnit=N, aalmcirc=V, aalmperp=V, bearM=11.0, bearMu=M, wpt=DEST, bearD=11.0, bearDu=M, bearS=11.0, bearSu=M)>',
    ...
    def testNMEA2(self):  # stream of NMEA v2.30 device (u-blox M6N)
        EXPECTED_RESULTS = (
            ...
            "<NMEA(GPGLL, lat=53.450671, NS=N, lon=-2.24026, EW=W, time=10:29:29, status=A, posMode=A)>",
            "<NMEA(GPRMC, time=10:29:30, status=A, lat=53.450672, NS=N, lon=-2.240258, EW=W, spd=0.099, cog=, date=2021-03-07, mv=, mvEW=, posMode=A)>",
        )
    ...
    def testMIXED(self):  # stream of mixed NMEA & UBX data with nmea_only set to FALSE - should be ignored
        EXPECTED_RESULTS = (
            "<NMEA(GNGGA, time=10:41:13, lat=53.450593, NS=N, lon=-2.240372, EW=W, quality=1, numSV=5, HDOP=8.68, alt=65.4, altUnit=M, sep=48.5, sepUnit=M, diffAge=, diffStation=)>",
            ...
            "<NMEA(GAGSV, numMsg=1, msgNum=1, numSV=0, signalID=7)>",
            "<NMEA(GBGSV, numMsg=1, msgNum=1, numSV=0, signalID=1)>",
            "<NMEA(GNGGA, time=10:41:14, lat=53.450593, NS=N, lon=-2.240361, EW=W, quality=1, numSV=5, HDOP=8.68, alt=65.2, altUnit=M, sep=48.5, sepUnit=M, diffAge=, diffStation=)>",
    ...
    def testMIXED2(self):  # stream of mixed NMEA & UBX data with nmea_only set to TRUE - should be rejected
